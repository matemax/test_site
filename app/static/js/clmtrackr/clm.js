...
var facew=0;
var faceh=0;
var eye1x, eye1y, eye1w, eye1h;
var eye2x, eye2y, eye2w, eye2h;
var nosex, nosey, nosew, noseh;
// These are also referenced in track.js D:
var score = 0;
var radnose = 0;
var overlayCC_;
var overlay_;
	...
	tracker : function(params) {
		...
		*/
		...
		this.track = function(element, box) {
			...
			if ( (scoringWeights && (facecheck_count % 10 == 0)) || (need_reset= true))  {
				if (!checkTracking()) {
		...
		this.draw = function(canvas, pv, path) {
			...
			var scoreThreshold = document.getElementById('iscore').value;

			// var normalColors =		["#FF0000", "#FF6600", "#FF9966", "#CCFFFF"];
			// var highlightColors = ["#FF6666", "#FFA366", "#FFC2A3", "#EBFFFF"];
			// var glowColors =			["#FF3333", "#FF8533", "#FFAD85", "#75FFFF"];
			//
			// var minScore = 0.6;
			// var maxScore = scoreThreshold;
			// var remappedScore = Math.min(maxScore, score) - minScore / (maxScore - minScore);
			//
			// var colorIndex = Math.round((remappedScore * 4) / 4);
			//
			// console.log("colorIndex %d", colorIndex);

			var highlightColor = "#EBFFFF";
			var normalColor = "#CCFFFF";
			var glowColor = "#75FFFF";
			var contrastColor = "#CCFF33";

			if(score < scoreThreshold){
				highlightColor = "#FF4719";
				normalColor = "#FF3300";
				glowColor = "#FF8566";
			}

			var cc = canvas.getContext('2d');
			cc.fillStyle = highlightColor;
			cc.strokeStyle = normalColor;
			cc.shadowBlur = glowSize;
			cc.shadowColor = glowColor;
			cc.lineWidth = 1.5;
			...
			//  if (score > document.getElementById('iscore').value &&
			//  		radnose < document.getElementById('irad').value) {
				...
				if(searching) {
					...
					if(dt >= 75) {
						partCounter = partCounter + animDir;
						prevTime = curTime;
					}

					if(partCounter > paths.length - 1) {
						partCounter = paths.length - 1;
						animDir *= -1;
					}
			...
			//}
			// else {
			// 	cc.strokeStyle = contrastColor
			// 	cc.shadowBlur = 0;
			// 	cc.strokeRect(facex,facey,facew,faceh);
			// 	cc.stroke();
			// }
		}
...
/*
                       var jf = new jsfeat_face(canvas);
                        ...
                        for (var i = 1; i < comp2.length; i++) {
                                if (comp2[i].confidence > candidate.confidence) {
                                        candidate = comp2[i];
                                }
                        }
facex1= candidate.x;
facey1= candidate.y;
facew1= candidate.width;
faceh1= candidate.height;
*/

        	if (!detector) {
                var width = 64; // ~~(120 * video.videoWidth / video.videoHeight);
                var height = 48;
		      	detector = new objectdetect.detector(width, height, 1.1, objectdetect.frontalface);
            	}

				var coords = detector.detect(canvas, 3);
...
//			console.log("detect");
				var coord = coords[0];
					...
					coord[0] *= video_.videoWidth / detector.canvas.width;
					coord[1] *= video_.videoHeight / detector.canvas.height;
					coord[2] *= video_.videoWidth / detector.canvas.width;
					coord[3] *= video_.videoHeight / detector.canvas.height;
					var kk = document.getElementById('ifoto').value;
					var qqq = Math.round(coord[2]/kk);
					facex= Math.round(coord[0]+ coord[2]/2  - qqq/2);
					facey= Math.round(coord[1]+ coord[3]/2 - qqq/2);
					facew= qqq;
					faceh= qqq;
				//}
			            candidate = {x : facex ,
...
//			console.log("lost");
		...
		var getInitialPosition = function(element, box) {
			...
			if (model.hints && mosseFilter && left_eye_filter && right_eye_filter && nose_filter) {
				...
				var yk = document.getElementById('ieye').value;
                                var nk = document.getElementById('inose').value;

				var noseFilterWidth = candidate.width * nk/10;
				var eyeFilterWidth = candidate.width * yk/10;
...
eye1x = Math.round(candidate.x+(candidate.width*3/4)-(eyeFilterWidth/2));
eye1y = Math.round(candidate.y+candidate.height*(2/5)-(eyeFilterWidth/2));
eye1w = eyeFilterWidth;
eye1h = eyeFilterWidth;


eye2x = Math.round(candidate.x+(candidate.width*1/4)-(eyeFilterWidth/2));
eye2y = Math.round(candidate.y+candidate.height*(2/5)-(eyeFilterWidth/2));
eye2w = eyeFilterWidth;
eye2h = eyeFilterWidth;

nosex = Math.round(Math.round(candidate.x+(candidate.width/2)-(noseFilterWidth/2)));
nosey = Math.round(candidate.y+candidate.height*(5/8)-(noseFilterWidth/2));
nosew = noseFilterWidth;
noseh = noseFilterWidth;
...
/*
				...
				canvasContext.strokeRect(left_eye_position[0], left_eye_position[1], eyeFilterWidth, eyeFilterWidth);
