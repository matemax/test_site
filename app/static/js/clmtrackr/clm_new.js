...
var clm = {
		...
		this.draw = function(canvas, pv, path) {
			...
			if (path === undefined) {
				...
				paths = model.path[path];
 				...
 				cc.lineWidth = '1';
                                      cc.fillStyle = 'rgba(0, 25, 255, 0.5)';
                                      cc.strokeRect( facex,facey,facew,faceh);

                                        cc.strokeRect( eye1x, eye1y, eye1w, eye1h);
                                        cc.strokeRect( eye2x, eye2y, eye2w, eye2h);
                                        cc.strokeRect( nosex, nosey, nosew, noseh);
	...
	var detectPosition = function(el) {
		...
		if ( coords[0]) {
			...
			coord[0] *= el.width / detector.canvas.width;
			coord[1] *= el.height / detector.canvas.height;
			coord[2] *= el.width / detector.canvas.width;
			coord[3] *= el.height / detector.canvas.height;
					
					 var kk = 1.5; //.5; // document.getElementById('ifoto').value;
...
*/
		...
		var getInitialPosition = function(element, box) {
			...
			if (model.hints && mosseFilter && left_eye_filter && right_eye_filter && nose_filter) {
				...
				var yk = 5;   // document.getElementById('ieye').value;
                                var nk = 4.5; // document.getElementById('inose').value;
